...
class MenuFilter {
    ...
    buildVegItems() {
        return [
            ...
            { id: 5, name: 'Palak Paneer', desc: 'Soft paneer in velvety spinach gravy, packed with greens and flavor.', price: 10.49, category: 'veg', subcategory: 'main', imageDesc: 'Creamy green palak paneer with paneer cubes and light cream drizzle in an elegant bowl.', placeholderPath: 'assets/images/menu/palak.jpg' },
    ...
    renderMenuItems() {
        ...
        const cards = this.menuItemsData.map((item) => {
            ...
            // Fallback image logic for all menu items
            let fallback = "assets/images/image-unavailable.png";
            let imgTag = `<img src="${item.placeholderPath}" alt="${item.imageDesc}" style="width: 100%; height: 100%; object-fit: contain; object-position: center; display: block;" loading="lazy" onerror=\"this.onerror=null;this.src='${fallback}';\" />`;
            // Special fallback for Palak Paneer (to its own image if available)
            if (item.name === 'Palak Paneer') {
                imgTag = `<img src="${item.placeholderPath}" alt="${item.imageDesc}" style="width: 100%; height: 100%; object-fit: contain; object-position: center; display: block;" loading="lazy" onerror=\"this.onerror=null;this.src='assets/images/dish/veg-palak-paneer.jpg';\" />`;
            }
...
class ThemeManager {
    ...
    init() {
        const savedTheme = localStorage.getItem(this.storageKey) || 'theme-4';
